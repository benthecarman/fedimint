...
async fn pay_invoice(
    ...
    ln_module
        .pay_bolt11_invoice(gateway, invoice, override_amount, ())
        .await
}
...
async fn test_can_attach_extra_meta_to_receive_operation() -> anyhow::Result<()> {
    ...
    let OutgoingLightningPayment {
        ...
        contract_id: _,
        fee: _,
    } = pay_invoice(&client2, invoice, None, None).await?;
    match payment_type {
...
async fn cannot_pay_same_internal_invoice_twice() -> anyhow::Result<()> {
    ...
    let OutgoingLightningPayment {
        ...
        contract_id: _,
        fee: _,
    } = pay_invoice(&client2, invoice.clone(), None, None).await?;
    match payment_type {
        PayType::Internal(op_id) => {
            let mut sub2 = client2
                .get_first_module::<LightningClientModule>()
...
async fn gateway_protects_preimage_for_payment() -> anyhow::Result<()> {
    ...
    let OutgoingLightningPayment {
        ...
        contract_id: _,
        fee: _,
    } = pay_invoice(&client1, invoice.clone(), None, Some(gw.gateway.gateway_id)).await?;
    match payment_type {
    ...
    let OutgoingLightningPayment {
        ...
        contract_id: _,
        fee: _,
    } = pay_invoice(&client2, invoice.clone(), None, Some(gw.gateway.gateway_id)).await?;
    match payment_type {
        PayType::Lightning(operation_id) => {
            let mut sub = client2
                .get_first_module::<LightningClientModule>()
...
async fn cannot_pay_same_external_invoice_twice() -> anyhow::Result<()> {
    ...
    let OutgoingLightningPayment {
        ...
        contract_id: _,
        fee: _,
    } = pay_invoice(&client, invoice.clone(), None, Some(gw.gateway.gateway_id)).await?;
    match payment_type {
        PayType::Lightning(operation_id) => {
            let mut sub = client
                .get_first_module::<LightningClientModule>()
    ...
    let OutgoingLightningPayment {
        ...
        contract_id: _,
        fee: _,
    } = pay_invoice(&client, invoice, None, Some(gw.gateway.gateway_id)).await?;
    match payment_type {
        PayType::Lightning(operation_id) => {
            let mut sub = client
                .get_first_module::<LightningClientModule>()
...
async fn can_pay_partial_invoice() -> anyhow::Result<()> {
    ...
    let OutgoingLightningPayment {
        ...
        &client1,
        invoice.clone(),
        Some(Amount::from_sats(70)),
        Some(gw.gateway.gateway_id),
    )
    .await?;
...
async fn makes_internal_payments_within_federation() -> anyhow::Result<()> {
    ...
    let OutgoingLightningPayment {
        ...
        contract_id: _,
        fee: _,
    } = pay_invoice(&client2, invoice, None, Some(gw.gateway.gateway_id)).await?;
    match payment_type {
        PayType::Internal(op_id) => {
            let mut sub2 = client2
                .get_first_module::<LightningClientModule>()
...
async fn rejects_wrong_network_invoice() -> anyhow::Result<()> {
    ...
    let error = pay_invoice(&client1, signet_invoice, None, Some(gw.gateway.gateway_id))
        .await
        .unwrap_err();
    assert_eq!(
        error.to_string(),
